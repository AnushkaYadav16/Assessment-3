...
def get_latest_table_name(database_name, table_prefix):
    ...
    Args:
        ...
        table_prefix (str): Prefix to filter table names.

    Returns:
        str: Name of the latest table, or None if not found.
    """
    try:
...
def lambda_handler(event, context):
    ...
    if state == "SUCCEEDED":
        ...
        # Determine crawler and table prefix
...
#         tables = glue.get_tables(DatabaseName=database_name)['TableList']
#         filtered_tables = [
#             table for table in tables if table['Name'].startswith(table_prefix)
#         ]
#         if not filtered_tables:
#             logger.error(f"No tables found with prefix '{table_prefix}' in DB '{database_name}'")
#             return None

#         latest_table = sorted(filtered_tables, key=lambda t: t['CreateTime'], reverse=True)[0]
#         return latest_table['Name']
#     except Exception as e:
...
#     if not job_name:
#         logger.warning("Job name missing in event")
#         return

#     if state == "SUCCEEDED":
#         logger.info(f"Glue job {job_name} succeeded. Triggering crawler...")
...
#             table_prefix = "txt"
#         else:
#             logger.warning(f"No crawler defined for job: {job_name}")
#             return

#         # Start the crawler
#         start_crawler_if_not_running(crawler_name)

#         # Wait for crawler to complete — consider using backoff or polling if needed
#         # Optional enhancement here if needed

#         # Get latest matching table
#         table_name = get_latest_table_name("processed_data_db", table_prefix)
#         if not table_name:
#             return

#         # Build and run Athena query
#         view_name = f"processed_{table_prefix.strip('_')}"
#         query = f"""
#         CREATE OR REPLACE VIEW {view_name} AS
#         SELECT * FROM processed_data_db.{table_name};
...
#                 }
#             )
#             logger.info(f"Athena view creation started for table: {table_name} | QueryExecutionId: {response['QueryExecutionId']}")
#         except Exception as e:
...
#     else:
#         logger.info(f"Glue job {job_name} in state: {state}")
